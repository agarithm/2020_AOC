...
class DB_UTILS extends Singleton {
	public function __construct(){
		global $wpdb;
		//HACK: Expose protected mysqli DB Handle from inside $wpdb
		...
		$this->DB_SLOW_QUERY_THRES = 10;
		$this->DB_UTILS_TABLE_LIST = null;
	...
	public static function Query($query,$trace=false){
		...
		if(($DB->DB_DEBUG||$trace) && (mysqli_errno($DB->DB)>0||mysqli_warning_count($DB->DB)>0)){
			if(mysqli_errno($DB->DB)>0){
				ERROR("DB: $query ");
				ERROR("DB: ERROR - ".mysqli_error($DB->DB)." (".mysqli_errno($DB->DB).") with following SQL: $query ");
				ERROR("DB: ERROR STACK TRACE\n".TEXT_STACK());
			}
			if(mysqli_warning_count($DB->DB)){
				if ($warn = mysqli_query($DB->DB, "SHOW WARNINGS")) {
					while($row = mysqli_fetch_row($warn)){
						...
						WARN(RenderTextArray($row,"DB WARNINGS"));
					}
				}
			}
		}else if($DB->DB_DEBUG||$trace){
			TRACE("DB: <pre> $query </pre>");
		}
		...
		if($qTime>$thres){
			WARN("SLOW: Query ($qTime seconds): $query");
			//Set Flag
	...
	private static function FixObjectErrors($obj,$class,$tableName,$primaryKeyName,$callback){
		...
		$getColumnType = function ($value){
			...
			case "double":
				...
				break;
			default:
				$out .= " text character set utf8mb4";
				break;
			}
		...
		if(isset($DB->DB)&&isset($DB->DB_AUTO_INSTALL)&&$DB->DB_AUTO_INSTALL){
			...
			case "1054"://Missing Column Name
				...
				if(count($currentCols)){
					foreach (get_class_vars($class) as $key => $value) {
						if(!in_array($key, $currentCols)){
							...
							if(static::Query($cmd,true))return static::$callback($obj,$class,$tableName,$primaryKeyName,true); //there may be more missing columns, allow it to recurse again;
						}
					}
				}else{
				...
				break;
			case "1300"://Wrong Encoding???
			case "1366"://Wrong Encoding???
				TRACE("Attempting to fix DB Schema: Converting to utf8mb4");
				$cmd = "ALTER TABLE `$tableName` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci";
				if(static::Query($cmd,true))return static::$callback($obj,$class,$tableName,$primaryKeyName, false); //	dont fix it again, if this didn't work, it wont work.
				break;
			default:
	...
	public static function InsertObject($obj,$class,$tableName,$primaryKeyName,$allowFixup=true){
		$DB = static::instance();
		...
		if( !$result && $allowFixup )$result = static::FixObjectErrors($obj,$class,$tableName,$primaryKeyName,"InsertObject");
		elseif (!$result) return 0; //tried the fixup once already.  Don't Loop.
	...
	public static function CreateTableFromClass($class,$table,$primaryKey){
		...
		if(isset($DB->DB_AUTO_INSTALL)&&$DB->DB_AUTO_INSTALL&&!static::IsATable($table)){
			...
			$cmd .= " CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci ENGINE=InnoDB";
