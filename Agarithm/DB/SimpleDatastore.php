...
abstract class SimpleDatastore {
	...
	public function __construct($className,$tableName,$primaryKey,$allowTags=false){
		...
		DB_UTILS::CreateTableFromClass($this->className,$this->tableName,$this->primaryKey);
	...
	public function lock(){
		...
		if(!$this->locked){
			$this->locked = true;
			DB_UTILS::Query("LOCK TABLES ".$this->tableName." WRITE");
		}
	}
	...
	public function save(){
		...
		if($this->$pk){
			DB_UTILS::UpdateObject($this,$this->className,$this->tableName,$this->primaryKey);
		}else{
			$this->$pk = DB_UTILS::InsertObject($this,$this->className,$this->tableName,$this->primaryKey);
		}
		return $this->$pk;
	}
	...
	public function delete(){
		$pk = $this->primaryKey;
		if($this->$pk){
			DB_UTILS::DeleteObject($this,$this->className,$this->tableName,$this->primaryKey);
	...
	public function search($key,$value){//pattern match
		return DB_UTILS::GetLikeObjects($this->className,$this->tableName,$key,$value);
	}

	public function match($key,$value,$sort="DESC"){//exact matches sorted by primaryKey
		return DB_UTILS::GetObjects($this->className,$this->tableName,$key,$value,$this->primaryKey,$sort);
	}
	...
	public function same($maybe){
		$rtn = true;
		if($rtn)$rtn &= method_exists($maybe,'varArray');
		if($rtn)$rtn &= isset($this->className);
		if($rtn)$rtn &= isset($maybe->className);
		if($rtn)$rtn &= Strings::Same($this->className,$maybe->className);
		if($rtn){
			foreach($this->varArray() as $key => $default){
				//don't compare primary keys
				if($key==$this->primaryKey)continue;
				if($rtn)$rtn &= isset($this->$key);
				if($rtn)$rtn &= isset($maybe->$key);
				if($rtn)$rtn &= Strings::Same($this->$key,$maybe->$key);
				//If not the same, break out and quit
				if(!$rtn)break;
			}
		}
		return $rtn;
	...
	public function count($key=false,$value=false){
		$out = 0;
		if($key)$key = DB_UTILS::Escape($key);
		if($key)$value = DB_UTILS::Escape($value);

		$cmd  = "SELECT count(*) as c FROM ".$this->tableName;
		if($key)$cmd .= " WHERE $key='$value'";
		$result = DB_UTILS::Query($cmd);
		if($result){
	...
	public function countLike($key,$value){
		$out = 0;
		$key = DB_UTILS::Escape($key);
		$value = DB_UTILS::Escape($value);
	...
	public function dropTable(){
		DB_UTILS::DropTable($this->tableName);
	}
	...
	public function rawEditForm($legend="",$div=false){
		$showPrevNext = function($self){
				...
				foreach($nextPart as $className => $id){
					if(DIRTY::GET($className))$url .= DIRTY::GET($className)."/";
					if(DIRTY::GET($className)==$self->className){
...
class sTest2 extends sTest{ //Flexes the Auto Install parts of the ORM
	...
	public function SelfTest(){
		...
		//HACK Test internals of DB_UTILS
		if(!DB_UTILS::AddObjectIndex($this->className,$this->tableName,"FKEY","ASC"))ERROR("FAIL: did not create index  on line ".__LINE__);
		if(!DB_UTILS::AddObjectIndex($this->className,$this->tableName,"NAME","DESC"))ERROR("FAIL: did not create index  on line ".__LINE__);
		if(!DB_UTILS::AddObjectIndex($this->className,$this->tableName,"NO_SORT"))ERROR("FAIL: did not create index  on line ".__LINE__);
		if(!DB_UTILS::AddObjectIndex($this->className,$this->tableName,"VALUE","DESC"))ERROR("FAIL: did not create index  on line ".__LINE__);
		if(!DB_UTILS::AddObjectIndex($this->className,$this->tableName,"FKEY","DESC"))ERROR("FAIL: did not create index  on line ".__LINE__);
